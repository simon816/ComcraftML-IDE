...
public class Parser {
    ...
    while (statement == null) {
      switch (state) {
        ...
        case 4:
          ...
          Vector<Expression> declarationVector = new Vector<Expression>();
  ...
  private Statement parseSwitchStatement() throws CompilerException {
    Vector<CaseStatement> caseClauseVector = new Vector<CaseStatement>();
    ...
    while (nextToken != Token.OPERATOR_CLOSEBRACE) {
      ...
      Vector<Statement> caseStatements = new Vector<Statement>();
  ...
  private Expression[] parseArgumentList() throws CompilerException {
    Vector<Expression> argumentVector = new Vector<Expression>();
  ...
  private Expression parsePrimaryExpression() throws CompilerException {
    ...
    //} else if (nextToken == Token.KEYWORD_IMPORT) {
    //    return null;
    //} else if (nextToken == Token.OPERATOR_ANNOTATION) {
    //    System.out.println("nextToken = " + nextToken);
    //    return parseAnnotationLiteral();
    } else {
...
private ArrayLiteral parseArrayLiteral() throws CompilerException {
    Vector<Expression> arrayElements = new Vector<Expression>();
  ...
  private FunctionLiteral parseFunctionLiteral(boolean nameFlag) throws CompilerException {
    ...
    Vector<Identifier> parameterVector = new Vector<Identifier>();
    Vector<Statement> statementVector = new Vector<Statement>();
  ...
  private ObjectLiteral parseObjectLiteral() throws CompilerException {
    Vector<ObjectLiteralProperty> propertyVector = new Vector<ObjectLiteralProperty>();
  ...
  private Expression parseAnnotationLiteral() throws CompilerException {
      readToken();
      Identifier anno = parseIdentifier();
      if (nextToken != Token.KEYWORD_FUNCTION) {
          throwCompilerException("function literal expected");
      }
      return new AnnotationLiteral(anno);
  }
